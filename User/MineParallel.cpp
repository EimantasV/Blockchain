...
string CalculateDifficulty(vector<Block> &Chain)
{
    ...
    {
        if ((*it).difficultyTarget != diffiulty)
            applicable = false;
...
std::atomic<bool> RunningThreads[32];
std::atomic_llong Nonces[32];

void ParallelMining(Block &newBlock, int threadID, int TotalThreads);
int main()
{
    int ThreadC;
    ...
    if (ThreadC <= 1)
    {
        cout << "Why are you even bothering?\n";
        system("pause");
        return 0;
    }
    string login;
    ...
    while (true)
    {
        ...
        RunningThreads[0] = true; // main
        std::thread Threads[32];

        for (int i = 1; i < ThreadC; i++) // run parallel, starts from 1 bcuz 0 is main thread.
        {
            RunningThreads[i] = true;
            Threads[i] = std::thread(ParallelMining, std::ref(newBlock), i, ThreadC);
            Threads[i].detach();
        }

        for (int i = 0; true; i += ThreadC) // actual mining
        {
            ...
            if (i % 100000 == 0)
            {
                ...
                {
                    RunningThreads[0] = false;
                    cout << "missed\n";
                    while (true)
                    {
                        bool done = true;
                        for (int b = 1; b < ThreadC; b++)
                        {
                            if (RunningThreads[b] == true)
                            {
                                done = false;
                            }
                        }
            ...
            if (solved)
            {
                RunningThreads[0] = false;
                Nonces[0] = i;
                cout << "Solved from: main" << endl;
            }

            for (int a = 0; a < ThreadC; a++)
            {
                if (RunningThreads[a] == false)
                {
                    newBlock.nonce = to_string(Nonces[a]);
                    solved = true;
                    break;
                }
            }
...
void ParallelMining(Block &newBlock, int threadID, int TotalThreads)
{
    ...
    for (int i = threadID; true; i += TotalThreads) // actual mining
    {
        for (int a = 0; a < TotalThreads; a++) // stop thread if others solved
        {
            if (RunningThreads[a] == false)
            {
                RunningThreads[threadID] = false;
                std::cout << "Thread broke: " << threadID << std::endl;
                return;
            }
        }
        ...
        if (solved)
        {
            std::cout << "solved from: " << threadID << std::endl;
            Nonces[threadID] = i;
            RunningThreads[threadID] = false;
            return;
        }
